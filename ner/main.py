...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate', type=float, default=l_rate, required=False, help='Set learning rate')
    parser.add_argument('--keep_prob', type=float, default=dropout_rate, required=False, help='Dropout_rate')
    ...
    if parameter["mode"] == "train":
        ...
        if embed == None:
            evalset.make_input_data(extern_data[train_lines:train_lines+eval_lines])
        else:
            evalset.make_input_str(extern_data[train_lines:train_lines+eval_lines])
