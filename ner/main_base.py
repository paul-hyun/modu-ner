...
if __name__ == '__main__':
    ...
    testset = Dataset(parameter, extern_data)
    ...
    if parameter["mode"] == "train":
        ...
        index_train = []
        index_test = []
        for index in range(0, len(extern_data)):
            if index % 13 == 1:
        ...
        for epoch in range(parameter["epochs"]):
            ...
            if 0 < len(index_test):
                avg_cost, avg_correct, precision_count, recall_count = iteration_model(model, testset, parameter, train=False)
                f1Measure, precision, recall = calculation_measure(precision_count, recall_count)
                if f1Max < f1Measure:
